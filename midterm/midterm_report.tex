...
The two models have been trained and compared for a $N$ DOF planar robot with $N=[2, 3, 4]$ in a fair setting, with same batch sizes and approximately the same amount of trainable parameters. The dataset consists of 1 million pairs of joint angles and end-effector position with a train / test split of 70 / 30. 

For comparison, two metrics are defined referring to \cite{Kruse2019}:

1 . The average mismatch between the true posterior $p_{gt}(x | y_{gt})$ obtained via rejection sampling and the predicted posterior $\tilde p(x | y_{gt})$ is computed for randomly but fixed chosen samples from the test dataset. For each test sample, 100 samples are drawn from the true posterior by rejection sampling and compared with 100 samples predicted by the model via MMD.

\begin{equation}
e_{posterior} = MMD(\tilde p(x | y_{gt}), p_{gt}(x | y_{gt}))
\end{equation}

2. The average re-simulation error is computed on the whole test dataset by applying the simulation for the true forward kinematics $y_{resim} = f( \tilde \theta)$ to the generated joint angles $\tilde \theta$ and measure the mean squared distance between the ground truth end-effector position $y_{gt}$ and the resimulated end-effector position $y_{resim}$ resulting from the predicted joint angles.
...
In Fig. \ref{fig:posterior:3dof} and Fig. \ref{fig:posterior:4dof}, the predicted and ground truth distributions of the joint angles given a specific end-effector position are shown, respectively.
